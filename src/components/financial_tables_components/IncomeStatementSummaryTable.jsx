...
export default function IncomeStatementSummaryTable() {
    ...
    const incomeStatementTitle = base_financial_dashboard.find((dt) => dt[0] == 10101)
    const incomeStatementValues = base_financial_dashboard.filter(
    ...
    return (
        <div className="w-full flex flex-col">
            <div className="pt-2 px-1 pb-1 text-red-600 border-b-4 border-red-600 text-lg">
                <strong>{incomeStatementTitle[1]}</strong>
            </div>
            <div className="w-full flex flex-col border-b-4 border-red-600">
                {incomeStatementValues.map((data, idx) => (
                    <div key={idx} className={`flex justify-between w-full p-1 ${data[0] == 619 ? 'font-bold' : null}`}>
